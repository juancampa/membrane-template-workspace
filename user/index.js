...
export const Sms = {
  configure: async ({ args }) => await sys_user.configureSms({ ...args }).$invoke(),
...
export const Email = {
  configure: async ({ args }) => await sys_user.configureEmail({ ...args }).$invoke(),
  handleEmail: async ({ args: { message } }) => await root.email.received.$emit({ message }),
}
